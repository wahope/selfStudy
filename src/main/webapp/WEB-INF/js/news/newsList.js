layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        elem: '#newsList',
        url : 'http://localhost:8080/BackNews/selectAllNews',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limit : 10,
        ...
        cols : [[
            ...
            {field: 'title', title: '动态标题', width:120,align:"center"},
            {field: 'specialtyId', title: '专业id', align:'center',width:80},
            {field: 'specialtyName', title: '专业名称', align:'center',width:100},
            {field: 'content', title: '动态内容', align:'center', width:270},
    ...
    function addNews(edit){
        ...
        var index = layui.layer.open({
            title : "修改网盘资料",
            type : 2,
            content : 'http://localhost:8080/BackNews/toUpdateNews?id='+ids,
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                if(edit){
                    body.find(".id").val(edit.id);
                    body.find(".title").val(edit.title);
                    // body.find("#specialty").val(edit.specialtyId);
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的文章？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("http://localhost:8080/BackNews/delMoreNews",{
    ...
    table.on('tool(newsList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除选中的文章？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("http://localhost:8080/BackNews/delNews",{
