...
//            console.log(data.value); //得到被选中的值
    ...
    form.on("submit(updateWeb)",function(data){
        //弹出loading
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});

        $.ajax({
            url:"http://localhost:8080/BackWeb/updateWeb",
            type:"post",
            dataType: 'json',
            data:{
                id:$(".id").val(),
                websitesName : $(".websitesName").val(),  //软件名称
                stageId : $("#stage").val(),  //阶段id
                websitesUrl : $(".websitesUrl").val(),  //下载地址
    ...
    form.on("submit(addWeb)",function(data){
        //弹出loading
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        console.log($("#stage").val());
        $.ajax({
            url:"http://localhost:8080/BackWeb/insertWeb",
            type:"post",
            dataType: 'json',
            data:{
                websitesName : $(".websitesName").val(),  //软件名称
