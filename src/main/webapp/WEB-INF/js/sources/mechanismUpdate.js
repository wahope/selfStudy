...
//            console.log(data.value); //得到被选中的值
    ...
    form.on("submit(updateMec)",function(data){
        //弹出loading
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});

        $.ajax({
            url:"http://localhost:8080/BackMec/updateMec",
            type:"post",
            dataType: 'json',
            data:{
                id:$(".id").val(),
                mechanismName : $(".mechanismName").val(),  //软件名称
                stageId : $("#stage").val(),  //阶段id
                mechanismUrl : $(".mechanismUrl").val(),  //下载地址
    ...
    form.on("submit(addMec)",function(data){
        //弹出loading
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        console.log($("#stage").val());
        $.ajax({
            url:"http://localhost:8080/BackMec/insertMec",
            type:"post",
            dataType: 'json',
            data:{
                mechanismName : $(".mechanismName").val(),  //软件名称
