layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        elem: '#newsList',
        url : 'http://localhost:8080/BackWeb/selectAllWeb',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limit : 10,
        ...
        cols : [[
            ...
            {field: 'websitesName', title: '网站名称', width:100,align:"center"},
            {field: 'stageId', title: '专业阶段', align:'center',width:80},
            {field: 'stageName', title: '阶段名称', align:'center',width:100},
            {field: 'websitesUrl', title: '网站地址', align:'center', width:270},
    ...
    function addNews(edit){
        ...
        var index = layui.layer.open({
            title : "修改网站信息",
            type : 2,
            content : 'http://localhost:8080/BackWeb/toUpdateWeb?id='+ids,
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                if(edit){
                    body.find(".id").val(edit.id);
                    body.find(".websitesName").val(edit.websitesName);
                    body.find(".stageId").val(edit.stageId);
                    body.find(".websitesUrl").val(edit.websitesUrl);
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的文章？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("http://localhost:8080/BackWeb/delMoreWeb",{
    ...
    table.on('tool(newsList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除选中的文章？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("http://localhost:8080/BackWeb/delWeb",{
