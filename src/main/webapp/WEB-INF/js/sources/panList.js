layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        elem: '#newsList',
        url : 'http://localhost:8080/BackPan/selectAllPan',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limit : 10,
        ...
        cols : [[
            ...
            {field: 'panTitle', title: '网盘资料标题', width:120,align:"center"},
            {field: 'stageId', title: '专业阶段', align:'center',width:100},
            {field: 'panUrl', title: '下载地址', align:'center', width:270},
    ...
    function addNews(edit){
        ...
        var index = layui.layer.open({
            title : "修改网盘资料",
            type : 2,
            content : 'http://localhost:8080/BackPan/toUpdatePan?id='+ids,
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                if(edit){
                    body.find(".id").val(edit.id);
                    body.find(".panTitle").val(edit.panTitle);
                    body.find(".stageId").val(edit.stageId);
                    body.find(".panUrl").val(edit.panUrl);
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的文章？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("http://localhost:8080/BackPan/delMorePan",{
    ...
    table.on('tool(newsList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除选中的文章？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("http://localhost:8080/BackPan/delPan",{
